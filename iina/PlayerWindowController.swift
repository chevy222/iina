...
class PlayerWindowController: NSWindowController, NSWindowDelegate {
  ...
  private func handleMappedLeftMouseEvent(_ event: NSEvent) {
    ...
    if event.clickCount == 1 {
      if hasDoubleClickBinding {
        // Wait briefly to see whether a second click arrives; if not, treat as single click.
    ...
    // Cancel the pending single click and dispatch the double-click binding.
    // If MBTN_LEFT_DBL is not bound we deliberately do nothing: the first click
    // already fired MBTN_LEFT (or will, via the timer), so falling back to it
    // here would cause the action to run twice.
    singleClickTimer?.invalidate()
    singleClickTimer = nil
    _ = handleInputBinding("MBTN_LEFT_DBL")
  }
  ...
  internal func performMouseAction(_ action: Preference.MouseClickAction) {
    // Subclasses (e.g. MainWindowController) extend this with fullscreen / PIP / etc.
    // The base implementation is kept so that the force-touch path, which still
    // dispatches by preference rather than via input.conf, has a fallback.
    switch action {
    case .pause:
      player.togglePause()
    default:
  ...
  override func scrollWheel(with event: NSEvent) {
    // `isDirectionInvertedFromDevice` corresponds to a "natural" scroll setting
    // (the same convention mpv uses for these axes). The two axes are dispatched
    // independently so that diagonal trackpad scrolls and Magic Mouse horizontal
    // scrolls are not silently dropped.
    ...
    if deltaY > 0 { _ = handleInputBinding("WHEEL_UP") }
    if deltaY < 0 { _ = handleInputBinding("WHEEL_DOWN") }
    if deltaX > 0 { _ = handleInputBinding("WHEEL_RIGHT") }
    if deltaX < 0 { _ = handleInputBinding("WHEEL_LEFT") }
  }
